...
This guide lets a Codex agent reuse the existing Claude-focused AI assets without touching the CI stack.
...
## 3. Respect Claude assets
- Treat everything under `.claude/` as your configuration source (settings, hooks, policies).
- Do not try to regenerate or fork those files; reuse them as-is.
- If you need tier guidance, assume the default Claude tier unless the user states otherwise.
...
## 5. Escalation rules
- Claude remains the primary agent. If you hit a policy conflict or need broader access, stop and hand off to the Claude tier.
- Record any questions or blockers in `TODO.md` if asked, then notify the user.

Following these steps keeps Codex aligned with the existing Claude-first workflow while giving it the context it needs to contribute safely.
