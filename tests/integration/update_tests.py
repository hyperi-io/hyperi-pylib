...
replacements = [
    ...
    (
        r'app_file\.write_text\(""".*?from hyperi_pylib import Application, create_metrics.*?app\.run\(\)\n"""\)',
        'app_file.write_text(self.load_fixture("test_container_deployment_code_2"))',
    ),
    # Daemon app
    (
        r'app_file\.write_text\(""".*?from hyperi_pylib import Application, get_mount_config.*?app\.run\(\)\n"""\)',
        'app_file.write_text(self.load_fixture("test_container_deployment_code_3"))',
    ),
    # Database compose app
    (
        r'app_file\.write_text\(""".*?from hyperi_pylib import get_database_config, build_database_url.*?print\(f"Database: \{db_config\[\'database\'\]\}"\)\n"""\)',
