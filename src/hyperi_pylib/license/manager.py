...
# File:      src/hyperi_pylib/license/manager.py
...
2. HYPERI_LICENSE_PATH environment variable
3. Standard paths (./license.enc, /etc/hyperi/license.enc, etc.)
4. HYPERI_LICENSE_URL environment variable (if httpx available)
...
class License:
    ...
    def _load_license(cls, opts: LicenseOptions) -> tuple[LicenseSettings, LicenseSourceInfo]:
        ...
        env_path = os.environ.get("HYPERI_LICENSE_PATH")
        if env_path:
            path = Path(env_path)
            if path.exists():
        ...
        url = opts.license_url or os.environ.get("HYPERI_LICENSE_URL")
        if url:
            try:
    ...
    def _standard_license_paths() -> list[Path]:
        ...
        # /etc/hyperi/
        paths.append(Path("/etc/hyperi/license.enc"))
