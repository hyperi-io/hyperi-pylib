...
"""Generate deployment artefacts from a ``DeploymentContract`` — mirrors
rustlib's ``hyperi_rustlib::deployment::generate``.
...
def generate_dockerfile(contract: DeploymentContract) -> str:
...
def generate_container_manifest(contract: DeploymentContract) -> str:
...
def generate_runtime_stage(contract: DeploymentContract) -> str:
...
def generate_chart(contract: DeploymentContract, output_dir: Path | str) -> None:
    ...
    _write_file(out / "Chart.yaml", _gen_chart_yaml(contract))
...
def _gen_chart_yaml(c: DeploymentContract) -> str:
...
class ArgocdConfig:
    ...
    sync_wave: int = 0
    """Sync wave (lower runs first)."""


def generate_argocd_application(contract: DeploymentContract, argo: ArgocdConfig) -> str:
