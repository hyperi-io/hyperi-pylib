...
class ResourceManager:
    ...
    def calculate_safe_thread_limits(self, resources: dict[str, Any]) -> dict[str, int]:
        ...
        if self.config.max_io_threads_override:
            io_threads = self.config.max_io_threads_override
        else:
            io_threads = min(max_threads_by_memory, max_threads_by_cpu, 32)
        
        if self.config.max_cpu_processes_override:
            cpu_processes = self.config.max_cpu_processes_override
        else:
            cpu_processes = max(1, int(cpu_cores))
        
        self.logger.info(f"🧵 Thread allocation:")
        self.logger.info(f"   I/O threads: {io_threads} (memory limit: {max_threads_by_memory}, CPU limit: {max_threads_by_cpu})")
...
class ContainerApp:
    ...
    def __init__(self, 
        ...
        logger.info(f"   Thread allocation: {self.thread_limits['io_threads']} I/O, {self.thread_limits['cpu_processes']} CPU")
    ...
    def _setup_thread_pools(self):
        ...
        self.io_thread_pool = ThreadPoolExecutor(
            max_workers=self.thread_limits["io_threads"],
            thread_name_prefix=f"{self.config.app_name}_io"
        )
        
        # CPU process pool for compute-intensive tasks
        self.cpu_process_pool = ProcessPoolExecutor(
            max_workers=self.thread_limits["cpu_processes"]
        )
        
        logger.info(f"🏊 Thread pools initialized")
    ...
    def _setup_memory_guardian(self):
        ...
        self.memory_guardian_thread = threading.Thread(
            target=memory_guardian,
            name=f"{self.config.app_name}_memory_guardian",
            daemon=True
        )
    ...
    def _create_default_api(self):
        ...
        try:
            ...
            app = FastAPI(
                ...
                description="Enterprise containerized application",
                version="1.0.0"
            )
    ...
    def _graceful_shutdown(self):
        ...
        try:
            ...
            if sys.version_info >= (3, 9):
                ...
                self.cpu_process_pool.shutdown(wait=True)
...
def create_daemon_app(business_logic: Any, 
                     app_name: str,
...
def create_api_daemon_app(business_logic: Any,
                         app_name: str, 
                         api_port: int = 8000,
                         **kwargs) -> ContainerApp:
...
def create_oneshot_app(task_func: Callable,
                      app_name: str,
                      **kwargs) -> Any:
