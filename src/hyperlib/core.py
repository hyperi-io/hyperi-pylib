...
def _build_log_format(config: dict) -> str:
    ...
    if log_format == "json":
        ...
        return '{{"timestamp":"{time:' + time_format + '}", "level":"{level}", "name":"{name}", "function":"{function}", "line":{line}, "message":"{message}"}}\n'
    elif log_format == "logfmt":
        # logfmt format (key=value pairs)
        if include_caller:
            return 'time={time:' + time_format + '} level={level} name={name} function={function} line={line} msg="{message}"\n'
        else:
        ...
        if use_color:
            if include_caller:
                return '<green>{time:' + time_format + '}</green> | <level>{level: <8}</level> | <cyan>{name}</cyan>:<cyan>{function}</cyan>:<cyan>{line}</cyan> - <level>{message}</level>\n'
            else:
                return '<green>{time:' + time_format + '}</green> | <level>{level: <8}</level> | <level>{message}</level>\n'
        else:
            if include_caller:
                return '{time:' + time_format + '} | {level: <8} | {name}:{function}:{line} - {message}\n'
            else:
