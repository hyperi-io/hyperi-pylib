...
Basic Usage:
    ...
    @app.command()
    def info():
        from hs_pylib.cli.version import get_version
...
def get_version(package_name: str, fallback: str = "unknown") -> str:
    ...
    Args:
        package_name: Name of the package (e.g., "hs_pylib", "myapp")
    ...
    Example:
        version = get_version("hs_pylib")
        print(f"hs-pylib version: {version}")
    """
    try:
...
def print_version(package_name: str, app_name: str | None = None, python_version: bool = False):
    ...
    Example:
        print_version("hs_pylib", app_name="hs-pylib CLI")
        # Output: hs-pylib CLI version 2.7.3
    """
    if app_name is None:
...
# Pre-configured version option for hs-pylib itself
HS_LIB_VERSION_OPTION = version_option("hs_pylib", app_name="hs-pylib", python_version=True) if HAS_TYPER else None
