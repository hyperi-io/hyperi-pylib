"""
hs-pylib Config - Enterprise Configuration with Automatic Cascade
===================================================================
...
**Multi-File Support:**
    ...
    hs-pylib auto-discovers and merges multiple config sources:

    1. Built-in defaults (hs-pylib internals)
...
def get_app_name() -> str:
    ...
    2. HS_LIB_APP_NAME override
    3. Root application package name (not hs_pylib)
    ...
    try:
        ...
        for dist in importlib.metadata.distributions():
            ...
            # Skip common libraries and hs_pylib itself
            if name and name not in ("hs_pylib", "pip", "setuptools", "wheel"):
...
if DETECTED_ENV in ["kubernetes", "docker", "container"]:
    ...
    if "/src/hs_pylib/" in str(current_file):
...
def init_config_directory(
    ...
    Example:
        from hs_pylib.config import init_config_directory
...
# Reference: https://docs.hs-pylib.io/config/targets
