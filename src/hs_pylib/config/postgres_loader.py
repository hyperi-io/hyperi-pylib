...
======================
    ...
    HS_CONFIG_FALLBACK_FILE="/config/fallback.yaml"
    ...
    HS_CONFIG_FALLBACK_MODE="replace"
...
class PostgresConfigLoader:
    ...
    def __init__(
        ...
        self.cache_ttl = (
            cache_ttl if cache_ttl is not None else int(os.getenv("HS_CONFIG_CACHE_TTL", str(self.DEFAULT_CACHE_TTL)))
        )
        self.connect_timeout = (
            connect_timeout
            if connect_timeout is not None
            else int(os.getenv("HS_CONFIG_CONNECT_TIMEOUT", str(self.DEFAULT_CONNECT_TIMEOUT)))
        )
        self.query_timeout = (
            query_timeout
            if query_timeout is not None
            else int(os.getenv("HS_CONFIG_QUERY_TIMEOUT", str(self.DEFAULT_QUERY_TIMEOUT)))
        )
        self.retry_attempts = (
            retry_attempts
            if retry_attempts is not None
            else int(os.getenv("HS_CONFIG_RETRY_ATTEMPTS", str(self.DEFAULT_RETRY_ATTEMPTS)))
        )
        self.retry_delay_ms = (
            retry_delay_ms
            if retry_delay_ms is not None
            else int(os.getenv("HS_CONFIG_RETRY_DELAY_MS", str(self.DEFAULT_RETRY_DELAY_MS)))
        )
        ...
        if optional is not None:
            ...
            opt_env = os.getenv("HS_CONFIG_OPTIONAL", "true").lower()
        ...
        if fallback_enabled is not None:
            ...
            fb_env = os.getenv("HS_CONFIG_FALLBACK_ENABLED", "false").lower()
            self.fallback_enabled = fb_env in ("true", "1", "yes")

        if fallback_file is not None:
            self.fallback_file = Path(fallback_file)
        else:
            fallback_file_env = os.getenv("HS_CONFIG_FALLBACK_FILE")
        ...
        self.fallback_mode = fallback_mode or os.getenv("HS_CONFIG_FALLBACK_MODE", self.DEFAULT_FALLBACK_MODE)
    ...
    def load_sync(self) -> dict[str, Any]:
        ...
        try:
            ...
            msg = "psycopg not installed, PostgreSQL config disabled. Install with: pip install hs-pylib[cache]"
            logger.warning(msg)
...
def load_postgres_config() -> dict[str, Any]:
    ...
    Environment Variables:
        ...
        HS_CONFIG_CONNECT_TIMEOUT: Connection timeout in seconds (default: 5)
        HS_CONFIG_QUERY_TIMEOUT: Query timeout in seconds (default: 10)
        HS_CONFIG_RETRY_ATTEMPTS: Retry attempts (default: 3)
        HS_CONFIG_RETRY_DELAY_MS: Retry delay in milliseconds (default: 1000)
        HS_CONFIG_OPTIONAL: Continue on failure (default: true)
