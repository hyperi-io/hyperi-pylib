...
class SecretsManager:
    ...
    def from_config(cls, config: dict[str, Any]) -> "SecretsManager":
        ...
        if "openbao" in config:
            try:
                ...
                logger.warning("OpenBao provider not available. Install with: pip install hs-pylib[secrets-vault]")
        ...
        if "aws" in config:
            try:
                ...
                logger.warning("AWS provider not available. Install with: pip install hs-pylib[secrets-aws]")
        ...
        cache_config = CacheConfig(
            ...
            directory=cache_cfg.get("directory") or os.environ.get("HS_SECRETS_CACHE_DIR"),
            ttl_secs=cache_cfg.get("ttl_secs", int(os.environ.get("HS_SECRETS_CACHE_TTL", "3600"))),
    ...
    def _get_encryption_key(cache_cfg: dict) -> bytes | None:
        ...
        key = cache_cfg.get("encryption_key") or os.environ.get("HS_SECRETS_CACHE_KEY")
        if key:
